...
public class Dataset {
	...
	private long [] itemFrequencies = null;
	...
	private void prepare() {
		try {
			...
			itemFrequencies = new long  [itemNumber + 1];
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void removeNonFrequentItems(float threshold) {		
		for (int i = 1; i < itemNumber + 1; i++)
			if (itemFrequencies[i] < threshold * transactionNumber) {
				for (int t = 0; t < transactionNumber; t++)
					if (matrix[t][i]) {
						matrix[t][i] = false;
						transactionLengths[t]--;
						size--;
					}
			}
	}
	...
	public int getItemFrequency(int i) {
		return (int) itemFrequencies[i];
	}

	public int getMaximumLength() {
		int maxlen = 0;
		for (int t = 0; t < transactionNumber; t++)
			if (transactionLengths[t] > maxlen)
				maxlen = transactionLengths[t];
		return maxlen;
	}
